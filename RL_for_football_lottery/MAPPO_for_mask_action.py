...
import numpy as np
from Buffer import Buffer_for_PPO_mask
...
class MAPPO: 
    ...
    ## buffer 相关
    def add(self, obs, action, reward, next_obs, done, action_log_pi , adv_dones,avail_actions):
        for agent_id, buffer in self.buffers.items():
            buffer.add(obs[agent_id], action[agent_id], reward[agent_id], next_obs[agent_id], done[agent_id], action_log_pi[agent_id] , adv_dones[agent_id], avail_actions[agent_id])
    ...
    def all(self):
        ...
        avail_actions = {}
        for agent_id, buffer in self.buffers.items():
            obs[agent_id], action[agent_id], reward[agent_id], next_obs[agent_id], done[agent_id], action_log_pi[agent_id], adv_dones[agent_id] ,avail_actions[agent_id] = buffer.all()
        return obs, action, reward, next_obs, done, action_log_pi, adv_dones , avail_actions
    ...
    def learn(self, minibatch_size, gamma, lmbda ,clip_param, K_epochs, entropy_coefficient, huber_delta = None):
        ...
        obs, action, reward, next_obs, done , action_log_pi , adv_dones , avail_actions = self.all()
...
if __name__ == '__main__':
    ...
    parser.add_argument("--learn_episode_interval", type=int, default=8) # 使用这个而不是 horizon minibatch_size
    # 训练参数
    ...
    # 此环境不进行observation归一化,因为环境里归一化过了 注意
    ...
    env = Env()
    env_info = env.get_env_info()
    dim_info = {agent_id : [env_info['obs_shape'] , env_info['n_actions']] for agent_id in env.agents_id}
    max_action = None
    is_continue = False
    ...
    win_list = [] # 胜利列表
    obs, info = env.reset() # env.reset(seed = args.seed)  # 针对obs复现:env.reset()
    ...
    while episode_num < args.max_episodes:
        ...
        avail_actions = info['action_mask']#env.get_avail_actions()  #avail_a_n.shape=(N,action_dim)
        ## 环境包装
        ...
        # 探索环境
        #reward, env_done, infos = env.step(action) 
        next_obs, reward, terminated,truncated, infos = env.step(action) # next_obs_n.shape=(N,obs_dim), reward.shape=(N,), env_done.shape=(N,), infos为字典
        ...
        if args.trick['reward_norm'] or args.trick['reward_scaling']:
            policy.add(obs, action, reward_, next_obs, done_bool, action_log_pi, done,avail_actions)
        else:
            policy.add(obs, action, reward, next_obs, done_bool, action_log_pi, done,avail_actions)
