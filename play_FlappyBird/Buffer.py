...
class Buffer_atari:
    ...
    def all(self,device):
        obs = torch.as_tensor(np.stack(self.env_buffer['obs'],axis= 0),dtype=torch.float32).to(device)
        actions = torch.as_tensor(np.stack(self.env_buffer['action'],axis= 0),dtype=torch.float32).to(device)
        rewards = torch.as_tensor(np.stack(self.env_buffer['reward'],axis= 0),dtype=torch.float32).reshape(-1,1).to(device)
        next_obs = torch.as_tensor(np.stack(self.env_buffer['next_obs'],axis= 0),dtype=torch.float32).to(device)
        dones = torch.as_tensor(np.stack(self.env_buffer['done'],axis= 0),dtype=torch.float32).reshape(-1,1).to(device)
        action_log_probs = torch.as_tensor(np.stack(self.env_buffer['action_log_pi'],axis= 0),dtype=torch.float32).reshape(-1,1).to(device)
        adv_dones = torch.as_tensor(np.stack(self.env_buffer['done_bool'],axis= 0),dtype=torch.float32).reshape(-1,1).to(device)    
