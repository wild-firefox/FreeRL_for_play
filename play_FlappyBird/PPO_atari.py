...
from Buffer import Buffer_atari
...
class Actor_discrete(nn.Module):
    def __init__(self, obs_dim, action_dim, hidden_1=100, hidden_2=100):
        super(Actor_discrete, self).__init__()
        self.l1 = nn.Linear(obs_dim, hidden_1)
        self.l2 = nn.Linear(hidden_2, action_dim)

    def forward(self, obs ):
        x = F.relu(self.l1(obs)) #
        a_prob = torch.softmax(self.l2(x), dim=1)
        return a_prob
    
class Critic(nn.Module):
    def __init__(self, obs_dim, hidden_1=100, hidden_2=100):
        super(Critic, self).__init__()
        self.l1 = nn.Linear(obs_dim, hidden_1)
        self.l2 = nn.Linear(hidden_2, 1)
...
class PPO:
    ...
    def evaluate_action(self, obs):
        obs = torch.as_tensor(obs,dtype=torch.float32).unsqueeze(dim = 0).to(self.device)
        # if self.is_continue:
    ...
    def save(self, model_dir):
        torch.save(self.agent.actor.state_dict(), os.path.join(model_dir,"PPO_actor.pt"))
        torch.save(self.agent.critic.state_dict(), os.path.join(model_dir,"PPO_critic.pt"))
    ...
    def load(dim_info, is_continue ,model_dir,trick=None):
        ...
        policy.agent.actor.load_state_dict(torch.load(os.path.join(model_dir,"PPO_actor.pt")))
        return policy
...
class Normalization:
    ...
    def __call__(self, x, update=True):
        ...
        return x
'''
7: 6上改 1.转灰度图方法 cv2.COLOR_BGR2GRAY -> cv2.COLOR_BGRA2GRAY（发现只改了firststep()1处）  2.zero区域涂成背景 3.hidden 200 -> 100

注意：use_before_model：表示是否使用之前的模型进行训练（网络结果必须一致才能进行）
'''
if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    # 环境参数t
    parser.add_argument("--env_name", type = str,default='flappybird') #"ALE/Freeway-v5") 
    # 共有参数
    parser.add_argument("--seed", type=int, default=100) # 0 10 100tt
    parser.add_argument("--max_episodes", type=int, default=int(20000)) #500
    parser.add_argument("--save_freq", type=int, default=int(100))
    ...
    # 新增
    parser.add_argument("--learn_episodes_interval", type=int, default=20)  
    # 训练参数
    ...
    if args.env_name == 'flappybird':
        dim_info[0] = 55*38 # 状态维度
        dim_info[1] = 2 # 动作维度
        D = 55*38 
        first_action = 1 
    ...
    use_before_model = False#True
    if use_before_model:
        script_dir = os.path.dirname(os.path.abspath(__file__)) # 当前脚本文件夹
        before_model_dir = script_dir + '\\results\\flappybird\\PPO_8'
        policy.agent.actor.load_state_dict(torch.load(os.path.join(before_model_dir,"PPO_actor.pt")))
        policy.agent.critic.load_state_dict(torch.load(os.path.join(before_model_dir,"PPO_critic.pt")))
    ...
    while episode_num < args.max_episodes:
        ...
        if episode_first_step  == True:
            obs  = env.first_step()#env.step(first_action)[0] 
            cur_x = prepro(obs)
