...
class Actor_discrete(nn.Module):
    def __init__(self, obs_dim, action_dim, hidden_1=200, hidden_2=200):
...
class Critic(nn.Module):
    def __init__(self, obs_dim, hidden_1=200, hidden_2=200):
...
if __name__ == '__main__':
    ...
    parser.add_argument("--max_episodes", type=int, default=int(7000)) #500
    ...
    while episode_num < args.max_episodes:
        ...
        if done:
            ...
            episode_num += 1
            #obs,info = env.reset(seed=args.seed)
            obs , info= env.reset() #25.4.26 ：发现之前PPO收敛快一部分原因是因为reset的seed固定了 去掉seed
            episode_reward = 0
