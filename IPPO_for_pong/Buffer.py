...
class ReplayBuffer:
    ...
    def get_training_data(self):
        ...
        for key in self.buffer.keys():
            if key == 'a_n':
                batch[key] = torch.tensor(self.buffer[key], dtype=torch.long).to(self.device)
            else:
                batch[key] = torch.tensor(self.buffer[key], dtype=torch.float32).to(self.device)
        return batch
    
class Buffer_atari:
    """replay buffer for each agent for multi-agent 
    save a episode 
    """
    ...
    def all(self):
        obs = torch.as_tensor(np.stack(self.env_buffer['obs'],axis= 0),dtype=torch.float32).to(self.device)
        actions = torch.as_tensor(np.stack(self.env_buffer['action'],axis= 0),dtype=torch.float32).to(self.device)
        rewards = torch.as_tensor(np.stack(self.env_buffer['reward'],axis= 0),dtype=torch.float32).reshape(-1,1).to(self.device)
        next_obs = torch.as_tensor(np.stack(self.env_buffer['next_obs'],axis= 0),dtype=torch.float32).to(self.device)
        dones = torch.as_tensor(np.stack(self.env_buffer['done'],axis= 0),dtype=torch.float32).reshape(-1,1).to(self.device)
        action_log_probs = torch.as_tensor(np.stack(self.env_buffer['action_log_pi'],axis= 0),dtype=torch.float32).reshape(-1,1).to(self.device)
        adv_dones = torch.as_tensor(np.stack(self.env_buffer['done_bool'],axis= 0),dtype=torch.float32).reshape(-1,1).to(self.device)    

        return obs, actions, rewards, next_obs, dones , action_log_probs , adv_dones 
