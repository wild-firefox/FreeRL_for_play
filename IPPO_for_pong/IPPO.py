...
class Critic(nn.Module):
    ...
    def __init__(self, dim_info:dict[str,list], hidden_1=128 , hidden_2=128,trick = None):
        ...
        # 智能体编号
        self.id = Critic.id_num
        Critic.id_num += 1
        '''  各自的obs_dim '''
        obs_dim = list(dim_info.values())[self.id][0] # 当前智能体的obs_dim
...
class IPPO: 
    def __init__(self, dim_info, is_continue, actor_lr, critic_lr, horizon, device, trick = None):        
        ...
        for agent_id, (obs_dim, action_dim) in dim_info.items():
            ...
            self.buffers[agent_id] = Buffer_atari(device = device)
    ...
    def learn(self, minibatch_size, gamma, lmbda ,clip_param, K_epochs, entropy_coefficient, huber_delta = None):
        ...
        for agent_id in self.agents.keys():
            ...
            buffer_len = len(obs) # buffer_len = self.horizon
            self.horizon = buffer_len
            # 计算GAE
            with torch.no_grad():  # adv and v_target have no gradient
                adv = np.zeros(self.horizon)
                gae = 0
                vs = self.agents[agent_id].critic(obs)
                vs_ = self.agents[agent_id].critic(next_obs)
    ...
    def save(self, model_dir):
        torch.save(
            ...
            os.path.join(model_dir, 'IPPO.pth')
        )
...
def make_dir(env_name,policy_name = 'DQN',trick = None):
    ...
    print('trick:',trick)
    # 确定前缀
    if trick is None or not any(trick.values()) or policy_name =='IPPO':
...
if __name__ == '__main__':
    ...
    # 环境参数
    parser.add_argument("--env_name", type = str,default="pong_v3") 
    ...
    # PPO独有参数
    parser.add_argument("--horizon", type=int, default=256) # 这里无此参数
    ...
    # trick参数
    parser.add_argument("--policy_name", type=str, default='IPPO') # IPPO / IPPO_simple
    ...
    if  args.policy_name == 'IPPO' or ((args.trick['lr_decay'] is False ) and all(value  for key, value in args.trick.items() if key not in ['reward_norm','lr_decay'])) :
        args.policy_name = 'IPPO'
    ...
    if args.policy_name == 'IPPO_simple' or (not any(args.trick.values())) : # if all(value is False for value in args.trick.values()):
        args.policy_name = 'IPPO_simple'
        for key in args.trick.keys():
            args.trick[key] = False
    
    ## 单独把ObsNorms设置为False,因为这里已经归一化了
    args.trick['ObsNorm'] = False
    args.trick['reward_scaling'] = False
    ...
    max_steps = 10000
    env = pong_v3.parallel_env(max_cycles=max_steps )#, render_mode="human") #100000   #(render_mode="human")
    ...
    ## 算法配置
    policy = IPPO(dim_info, is_continue, args.actor_lr, args.critic_lr, args.horizon, device, args.trick)
    ...
    win_list = []
    lose_list = []
    action_env = {agent_id: 0 for agent_id in env_agents} 
    episode_step = 0
    while episode_num < args.max_episodes:
        ...
        if is_continue:
            ...
            action_ = { agent_id: 2 if action_[agent_id] == 0 else 3 for agent_id in env_agents} # 离散动作空间

        ## 修复环境bug 一开始不击打到球就不会发球的错误
        ## 每隔200步,记录分数 , 若是分数不变化，就让score分高的一方发球
        #print(step,episode_score)     
        ...
        if step % 200 == 0 :
            if score_200:
                ...
                last_score = episode_score
        if pass_true:
            '''  如果超过10000步,正好发球的那步的trun为true,那么这一步step会报错,next_obs中的agents会变成空,所以要判断一下 '''
            pass_true = False
            pass
        else:
            # 探索环境
            ...
            truncated = {agent_id: True for agent_id in env_agents} if (episode_step +1) == max_steps else truncated
            ##修改reward bug （一方0 一方-1） 这个不对
            if  sum(reward.values()) != 0: 
                reward = {agent_id: 0 for agent_id in env_agents}
            ...
            # if reward[env_agents[0]] != 0 or reward[env_agents[1]] != 0: #测bug时用
        ...
        if any(done.values()):
            ...
            episode_first_step = True
            ## 计算胜率
            if episode_score[env_agents[0]] > episode_score[env_agents[1]]:
                win_list.append(1)
            elif episode_score[env_agents[0]] == episode_score[env_agents[1]]:
                win_list.append(0.5)
            else:
                win_list.append(0)
            
            win_rate = sum(win_list) / len(win_list) if len(win_list) < 100 else sum(win_list[-100:]) / 100
            #lose_rate = sum(lose_list) / len(lose_list) if len(lose_list) < 100 else sum(lose_list[-100:]) / 100
            ...
            for agent_id in env_agents:
                ...
                writer.add_scalar(f'score_{agent_id}', episode_score[agent_id], episode_num + 1)
                train_return[agent_id].append(episode_reward[agent_id])
            writer.add_scalar('win_rate', win_rate, episode_num + 1)
            #writer.add_scalar('lose_rate', lose_rate, episode_num + 1)
