...
'''
老版本的 有bug：如果超过10000步,正好发球的那步的trun为true 训练结果是IPPO_18
'''
## 环境
...
if __name__ == '__main__':
    ...
    env = pong_v3.parallel_env(max_cycles=10000, render_mode="human") #100000   #(render_mode="human")
    ...
    while episode_num < args.max_episodes:
        ...
        if step % 200 == 0 :
            if score_200:
                ...
                # 若是分数不变化，就让score分高的一方发球 (环境没问题的话会自己发球)
                if last_score[env_agents[0]] == episode_score[env_agents[0]] and last_score[env_agents[1]] == episode_score[env_agents[1]]:
                    ...
                    if episode_score[env_agents[0]] > episode_score[env_agents[1]]: # 右边分数高，向左发球
                        action_env = {agent_id: 5 for agent_id in env_agents}
                    else:
                        action_env = {agent_id: 4 for agent_id in env_agents}
                last_score = episode_score
                env.step(action_env) ## 发球得分不记录??

            # if episode_score[env_agents[0]] == 1 and episode_score[env_agents[1]] == 0: 
            #     action_ = {agent_id: 4 for agent_id in env_agents}
        ...
        if any(done.values()):
            ...
            if episode_score[env_agents[0]] > episode_score[env_agents[1]]:
                ...
                win_list.append(0)
            # elif sum(episode_score.values()) < 0:
            #     lose_list.append(1)
